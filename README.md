...
fastqc (optional)
cufflinks (optional)
ribotish (requires Python3, for Ribo-seq QC. Better to have it)
```
...
Activate the env if you use conda:
...
#Also, add PipelineHomeDir and the associated bin folder to PATH so
  pipeline scripts (including PipeRiboseq.sh) can be recognized:
PATH=$PATH:/path/to/PipelineHomeDir:/path/to/PipelineHomeDir/bin
```
...
This repository has prepared the files for mm10 and hg38. If you are working on other genomes, some critical files need to be prepared. For more information, see this [tutorial](https://github.com/sunyumail93/PipeRNAseq/blob/master/preprocessing/Preprocessing.md).

1, For the **Annotation** folder, download GTF file from the UCSC table browser. `reduced`: Only one location was chosen when one gene duplicates at multiple genomic loci.

2, `uniqMatching.txt` file contains one-to-one matching from transcript to gene name. This can be extracted from the GTF file.

3, For the **Index** folder, indexes are not included in this GitHub directory, but need to be created during set up. Please make sure the same STAR version is used to build the index and run the pipeline.

4, For the **Sequence** folder, `RefSeq.reduced.bed12.fa` was converted from `RefSeq.reduced.bed12` file using bedtools, and rRNA genes (rRNA affects salmon quantification TPM) have been removed. `genome.fa` and `ChromInfo.txt` files need to be downloaded from UCSC goldenpath. The fai index file is not required now since it will be generated by samtools when needed.
